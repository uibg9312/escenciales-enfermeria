...
// -------------------------------------------------------------
// Tab Switching Logic (Listeners añadidos, reemplaza onclick en HTML)
// -------------------------------------------------------------
...
// Añadir listeners a los botones de tabs
if (btnCalcTab && btnNandaTab) {
    btnCalcTab.addEventListener('click', () => showTab('calculator'));
    btnNandaTab.addEventListener('click', () => showTab('nanda'));
}
// Set the initial active tab
showTab('calculator'); // Inicia con la calculadora visible
...
(function() {
    ...
    const concSolutions = { NaCl3:{name:'NaCl 3% (30mL)',mEqNaAmp:15.4,volAmp:30},NaCl17_7:{name:'NaCl 17.7% (20mL)',mEqNaAmp:30,volAmp:20},Bic8_4:{name:'Bicarbonato 8.4% (100mL)',mEqNaAmp:100,volAmp:100}};
    ...
    window.renderInputs = function() {
        ...
        switch (t) {
            ...
            // ... (resto de los cases para generar inputs - sin cambios internos) ...
             ...
             case 'mix':
                 ...
                 htmlInputs += `<label for="baseSel">Solución base Salina</label><select id="baseSel" required><option value="" disabled selected hidden>— Elija base —</option>${options(baseSolutions)}<option value="custom">Otra...</option></select>`;
                 ...
                 htmlInputs += `<label for="concSel">Solución concentrada Salina</label><select id="concSel" required><option value="" disabled selected hidden>— Elija concentrado —</option>${options(concSolutions)}<option value="custom">Otra...</option></select>`;
        ...
        if (t === 'mix') {
             ...
             const objetivoTipoSelect = $("objetivoTipo");
    ...
    function calcMix() {
        // ... (Código original COMPLETO de calcMix - sin modificaciones) ...
         ...
         if (baseKey === 'custom') {
          ...
          } else { throw new Error("Seleccione una solución base válida."); }
         let c;
         const concKey = $('concSel')?.value;
         if (concKey === 'custom') {
              const customVol = numVal('customConcVol');
              const customPct = numVal('customConcPct');
              ...
              const mEqL = pct2meq(customPct);
              c = { name: `Concentrado Personalizado (${customPct}%, ${customVol}mL)`, mEqNaAmp: mEqL * customVol / 1000, volAmp: customVol };
          ...
          } else { throw new Error("Seleccione una solución concentrada válida."); }
         ...
         let targetMeqTotal;
         if (tipo === 'conc') {
              const targetConc = numVal('targetConc');
          ...
          if (targetMeqTotal === undefined || isNaN(targetMeqTotal) || targetMeqTotal < 0) { throw new Error('Objetivo de Na⁺ inválido o no calculado.'); }
